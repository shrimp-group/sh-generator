  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" @submit.native.prevent>
      <el-form-item prop="tempCode">
        <el-input v-model="queryParams.tempCode" placeholder="模板编码, 模糊搜索" clearable style="width: 200px" @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item prop="tempName">
        <el-input v-model="queryParams.tempName" placeholder="模板名称, 模糊搜索" clearable style="width: 200px" @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item>
    ...
    <el-table v-loading="loading" :data="dataList">
      ...
      <el-table-column label="模板编码" prop="tempCode" width="120"/>
      <el-table-column label="模板Key" prop="tempKey" width="120"/>
      <el-table-column label="模板名称" prop="tempName" width="160"/>
      <el-table-column label="文件后缀" prop="tempSubfix" width="120"/>
      <el-table-column label="模板描述" prop="tempDesc" min-width="120"/>
...
<script setup name="GenTemplate">
import { templatePage, templateRemove} from "@/api/template";
...
function getList() {
  loading.value = true;
  templatePage(queryParams.value).then(res => {
    const data = res.data;
...
function handleDelete(row) {
  proxy.$modal.confirm('是否确认删除 :"' + row.id + '"？').then(() => {
    templateRemove({id: row.id}).then(res => {
      getList();
