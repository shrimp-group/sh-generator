...
- Req 对象统一放在 `bean/req` 包下，Resp 对象统一放在 `bean/resp` 包下，Entity 对象统一放在 `bean/entity` 包下
...
后端开发中，禁止在循环中操作外部资源（MySQL、Redis、HTTP 接口等），必须优先使用批量操作。仅在无法转换为批量操作时，才允许循环操作，但必须添加注释说明原因。
...
// ✅ 正确：使用批量接口
List<OrderDTO> orders = orderClient.batchGetOrders(ids);
```

**例外：无法批量时允许循环，但必须注释说明原因**

```java
// 该第三方接口不支持批量查询，只能逐条调用
for (Long id : ids) {
    OrderDTO order = thirdPartyClient.getOrder(id);
    orders.add(order);
}
```
