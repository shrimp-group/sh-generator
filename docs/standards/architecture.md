...
| 循环操作外部资源 | 禁止在循环中操作 MySQL、Redis、HTTP 接口等外部资源，必须优先使用批量操作 | 避免网络IO放大、连接池耗尽 |
...
**说明**：在循环中操作外部资源（MySQL、Redis、HTTP 接口等）会导致网络 IO 放大、连接池耗尽等严重性能问题，必须优先使用批量操作。

| 外部资源 | 禁止操作 | 推荐替代 |
|----------|----------|----------|
| MySQL | 循环逐条查询/更新 | 批量查询（WHERE IN）、批量插入/更新 |
| Redis | 循环逐条 GET/SET | Pipeline、MGET/MSET |
| HTTP 接口 | 循环逐条调用 | 批量接口 |

**例外**：当外部资源确实不支持批量操作时，允许循环操作，但必须在循环前添加注释说明原因。
