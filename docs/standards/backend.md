...
| 目录/包名 | 职责 | 说明 |
|---|---|---|
| bean | 数据对象（Entity/Req/Resp） | 客户端模块的数据传输对象 |
...
for (Long orderId : orderIds) {
    OrderResp order = orderClient.getOrder(orderId);
    // ...
}

// ✅ 正确：使用批量接口
List<OrderResp> orders = orderClient.batchGetOrders(orderIds);
```

### 例外情况

当外部资源确实不支持批量操作时，允许循环操作，但必须在循环前添加注释说明原因：
