...
#### Req/Resp 规范
* Java: Req 对象是否放在 `bean/req` 包下并继承公共父类（IdReq、PageReq、UpdateReq、RemoveReq），Resp 对象是否放在 `bean/resp` 包下并继承公共父类（EntityResp）

#### Entity 包路径
* Java: Entity 类是否放在 `bean/entity` 包下

#### 参数校验
* Java: 参数校验是否使用了 `Assert.notNull()`、`Assert.hasText()` 等 Spring Assert 方法，应替换为 `@Valid` + Bean Validation 注解（如 `@NotNull`、`@NotBlank` 等）

#### 对象复制
* Java: 数据库映射类（Entity）是否使用了类内复制方法，其他对象转换是否统一使用了 `BeanUtil.cp`

#### 循环操作外部资源
* Java: 是否存在循环查询 MySQL、循环操作 Redis、循环调用外部接口的情况，应优先使用批量操作（WHERE IN、Pipeline/MGET、批量接口），例外情况需有注释说明
