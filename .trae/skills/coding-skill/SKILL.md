...
```
bean/
  ├── entity/     # 数据库映射实体（原 model/entity 并入 bean）
  ├── req/        # 请求对象，继承 PageReq / IdReq / UpdateReq / RemoveReq
  └── resp/       # 响应对象，继承 EntityResp
```

> Entity 类统一放在 `bean/entity` 包下，禁止使用 `model/entity` 或其他路径。
...
- **循环操作外部资源**：禁止在循环中操作 MySQL、Redis、HTTP 接口等外部资源，必须优先使用批量操作。当无法批量时允许循环，但必须注释说明原因
...
```java
// ❌ 错误：循环中查询 MySQL
public List<OrderResp> getOrdersByUserIds(List<Long> userIds) {
    List<OrderResp> result = new ArrayList<>();
    for (Long userId : userIds) {
        // 每次循环都发起一次 SQL 查询，N 条数据产生 N 次数据库访问
        List<OrderEntity> orders = orderRepository.findByUserId(userId);
        result.addAll(orders.stream().map(OrderResp::from).collect(Collectors.toList()));
    }
    return result;
}
...
// ✅ 正确：使用批量查询替代循环
public List<OrderResp> getOrdersByUserIds(List<Long> userIds) {
    log.info("getOrdersByUserIds called, userIds={}", userIds);
    // 一次查询获取所有数据，避免 N+1 问题
    List<OrderEntity> orders = orderRepository.findAllByUserIdIn(userIds);
    return orders.stream().map(OrderResp::from).collect(Collectors.toList());
}

// ✅ 正确：无法批量时允许循环，但必须注释说明原因
public void syncOrderStatus(List<Long> orderIds) {
    log.info("syncOrderStatus called, orderIds={}", orderIds);
    for (Long orderId : orderIds) {
        // 无法批量：第三方接口不支持批量查询，只能逐条调用
        OrderStatusResp statusResp = thirdPartyClient.queryStatus(orderId);
        orderRepository.updateStatus(orderId, statusResp.getStatus());
    }
}
```
